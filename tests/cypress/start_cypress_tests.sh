...
if [ "$NODE_ENV" == "dev" ]; then
  NODE_OPTIONS="--trace-warnings" npx cypress run --browser $BROWSER $HEADLESS --spec "./tests/cypress/tests/*.spec.js" --reporter cypress-multi-reporters  
elif [ "$NODE_ENV" == "debug" ]; then
  NODE_OPTIONS="--trace-warnings" npx cypress open --browser $BROWSER --config numTestsKeptInMemory=0
else 
  NODE_OPTIONS="--trace-warnings" cypress run --browser $BROWSER $HEADLESS --spec "./tests/cypress/tests/*.spec.js" --reporter cypress-multi-reporters
fi
